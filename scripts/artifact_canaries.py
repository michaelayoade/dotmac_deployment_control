...
extent below is the POST-`dc_0012` one, thirteen tables and 178 columns, because
...
* `database_catalogue_as_published` — the installed distribution publishes the
                             exact catalogue: module identity, all thirteen table
                             identities, all 178 columns by name, ordinal, type
...
# `0.1.0a7`'s HEADLINE is a source-owned `ModuleDatabaseCatalogContributionV1`
# publishing `mod_deploy`'s exact seven platform tables and 95 columns; the
# literal below is the POST-`dc_0012` extent, thirteen tables and 178 columns, and
...
CATALOGUE_TABLES: tuple[
    ...
    (
        ...
        (
            ...
            ("created_at", 17, _TS, False, "now()"),
            ("updated_at", 18, _TS, False, "now()"),
            # dc_0009, and NULLABLE: a row written before Foundation's
            # prestate encoding identity existed stays distinguishable, so it
            # can be refused as historical rather than read as current.
            ("incumbent_prestate_discriminator", 19, _V128, True, ""),
        ),
    ),
    (
        "rehearsal_grants",
...
def canary_database_catalogue_as_published(expect_version: str) -> str:
    ...
    * module identity — document schema and scope, distribution name and
      version, module code, release version, `mod_deploy`, and the `dc_0012`
      lineage head;
    * all thirteen table identities, in canonical order, with nothing missing and
      nothing extra;
    * all 178 columns by name, physical ordinal, PostgreSQL type identity AND
