...
def test_a13_records_verification_and_a_distinct_adoption_disposition() -> None:
    """Coordinates say what was published; these fields say what that means.

    The recorder is deliberately unable to decide either question, so a green
    record branch must prove that a human supplied both rather than inheriting
    a coordinates-only row whose missing disposition happens not to be read.
    """
    a13 = next(r for r in _published()["releases"] if r["version"] == "0.1.0a13")
    assert a13["status"] == "released"
    assert a13["pinnable"] is True
    assert a13["release_run"] == "34687474025"
    assert a13["verify_run"] == "34687542590"
    verification = a13["release_run_note"]
    for evidence in (
        "registry-served",
        "sha256",
        "read-only consumer",
        "FIFTEEN behavioural canaries",
        "tag_once.py",
    ):
        assert evidence in verification, evidence
    adoption = a13["adoption_note"]
    for evidence in (
        "resolve_current_root",
        "fingerprint_standing",
        "AttestationRootRefusal",
        "a12",
        "does not make Control authoritative in production",
    ):
        assert evidence in adoption, evidence
    assert "superseded_by" not in a13


def test_a14_records_independent_verification_and_bounded_adoption() -> None:
    """The recorder's coordinates require a human verification and adoption ruling."""
    a14 = next(r for r in _published()["releases"] if r["version"] == "0.1.0a14")
    assert a14["status"] == "released"
    assert a14["pinnable"] is True
    assert a14["release_run"] == "35859642778"
    assert a14["verify_run"] == "35859932041"
    assert a14["peeled_commit"] == "f555f4384fbeb7eb18822bdaff70055fedbe1480"
    assert a14["tag_object"] == "9769096c2129589d4818d8ba2357111c6b249d1f"
    verification = a14["release_run_note"]
    for evidence in (
        "build run 35859642778",
        "INDEPENDENT verify run 35859932041",
        "publisher read-back",
        "exact filename",
        "sha256",
        ...
        "tag_once.py",
        "recorder then opened PR #64",
    ):
        assert evidence in verification, evidence
    adoption = a14["adoption_note"]
    for obligation in (
        "PIN a14",
        "ADR-0073 three-phase authenticated host admission",
        "Control-owned protected rehearsal-issuer issuance, revocation, "
        "staged consumption and standing",
        "a13 remains immutable, verified and pinnable",
        "does not activate an issuer",
        "Control Plane signer/verifier/harness composition",
        "prove Lane 3",
        "allocate a Foundation successor",
        "retire the incompatible Control Plane caller-derived/witness path",
    ):
        assert obligation in adoption, obligation
    assert "superseded_by" not in a14


def test_a15_records_independent_verification_and_bounded_adoption() -> None:
    """The recorder's coordinates require a human verification and adoption ruling."""
    a15 = next(r for r in _published()["releases"] if r["version"] == "0.1.0a15")
    assert a15["status"] == "released"
    assert a15["pinnable"] is True
    assert a15["release_run"] == "36162943891"
    assert a15["verify_run"] == "36163228780"
    assert a15["peeled_commit"] == "cd887722b3ffc909d98ef624032fd19cf31ba497"
    assert a15["tag_object"] == "466f48d2b9a75f6dc46aacb641f9e48c9a37ab8c"
    verification = a15["release_run_note"]
    for evidence in (
        "build run 36162943891",
        "INDEPENDENT verify run 36163228780",
        "publisher read-back",
        "exact filename",
        "sha256",
        "clean dependency-complete read-only consumer",
        ...
        "FIFTEEN installed-artifact behavioural canaries",
        "tag_once.py",
        "recorder then opened PR #68",
    ):
        assert evidence in verification, evidence
    adoption = a15["adoption_note"]
    for obligation in (
        "COMPATIBILITY RULING, NOT A RUNTIME ADOPTION CLAIM",
        "still pins 0.1.0a6",
        "PIN a15",
        "frozen Control plan purpose",
        "PLAN_PURPOSE_INCONSISTENT",
        "single Foundation V3 execution consumption",
        "stop every 0.1.0a14 process before migration dc_0015_plan_purpose runs",
        "NOT enforced by any deployment path",
        "a14 remains immutable, verified and pinnable",
        "does not activate an issuer",
        "prove Lane 3",
        "allocate a Foundation successor",
        "retire the incompatible Control Plane caller-derived/witness path",
